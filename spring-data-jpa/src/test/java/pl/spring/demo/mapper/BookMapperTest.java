...
public class BookMapperTest {
	...
	public void testShouldConvertBookTo() {
		...
		// when
		BookEntity bookEntityExpected = new BookEntity(1L, "Metro 2035",
				Arrays.asList(new AuthorTo(1L, "Dmitry", "Glukhovsky")));
	...
	public void testShouldConvertBookEntity() {
		// given
		BookEntity bookEntity = new BookEntity(1L, "Koniec swiata",
				Arrays.asList(new AuthorTo(1L, "Anna", "Mikolajewicz"), new AuthorTo(2L, "Wieslaw", "Lubelski")));
		// when
	...
	public void testShouldConvertBookEntityList() {
		// given
		BookEntity book1 = new BookEntity(1L, "Koniec swiata",
				Arrays.asList(new AuthorTo(1L, "Anna", "Mikolajewicz"), new AuthorTo(2L, "Wieslaw", "Lubelski")));
		BookEntity book2 = new BookEntity(2L, "Poczatek swiata", Arrays.asList(new AuthorTo(3L, "Anna", "Waszkiewicz"),
				new AuthorTo(4L, "Jerzy", "Zdrojewski"), new AuthorTo(5L, "Maciej", "Krychowiak")));
		List<BookEntity> books = Arrays.asList(book1, book2);
		// when
		List<BookTo> booksToExpected = Arrays.asList(
				new BookTo(1L, "Koniec swiata", "1 Anna Mikolajewicz,2 Wieslaw Lubelski,"),
	...
	public void testShouldConvertBookToList() {
		...
		// when
		List<BookEntity> bookEntityExpected = Arrays.asList(
				new BookEntity(1L, "Metro 2035", Arrays.asList(new AuthorTo(1L, "Dawid", "Glukhovsky"))),
		...
		assertEquals(bookEntityExpected.get(0).getAuthors().get(0).getFirstName(),
				result.get(0).getAuthors().get(0).getFirstName());
	}
